...
>
	<div
		role="presentation"
		class="bg-card rounded-card shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto pointer-events-auto"
		onclick={(e) => e.stopPropagation()}
	>
		<div class="relative">
			<div class="aspect-[4/3] bg-bone overflow-hidden rounded-t-card">
		...
		<div class="p-6 space-y-5">
			...
			<div class="bg-bone rounded-btn p-4">
				<p class="text-sm text-muted mb-1">Precio</p>
				<p class="text-2xl font-bold text-ember">{formatPrice(product.priceUSD, 'CUP')}</p>
			</div>

			<div class="flex gap-3">
				<button
					onclick={buyNow}
					class="flex-1 bg-ember text-white px-5 py-3 rounded-btn text-base font-medium transition-all duration-200 hover:bg-ember-active active:scale-[0.98] cursor-pointer flex items-center justify-center gap-2"
				>
					Comprar
				</button>
				<button
					onclick={() => { cart.addItem(product.id); onclose(); }}
					class="flex-1 px-5 py-3 border border-hairline text-body rounded-btn text-base font-medium transition-all duration-200 hover:bg-bone cursor-pointer flex items-center justify-center gap-2"
				>
